...
if ($GLOBALS["USER"]->IsAuthorized() && Loader::includeModule("socialnetwork"))
{
	...
	{
		$arMenuB24[] = [
			$isNewProjectsOn ? GetMessage("TOP_MENU_TASKS_NEW") : GetMessage("TOP_MENU_TASKS"),
			SITE_DIR . "tasks/menu/",
			[],
			[
				"name" => "tasks",
...
if (Loader::includeModule("socialnetwork"))
{
	...
	{
		$projectSubLink = SITE_DIR . "company/personal/user/" . $userId . "/groups/create/";
	}

	$groupsMessageKey = ($isNewProjectsOn ? 'TOP_MENU_PROJECTS' : 'TOP_MENU_GROUPS');
	...
	$arMenuB24[] = [
		...
		] + ($canCreateGroup ? ["sub_link" => $projectSubLink] : []),
...
{
	$arMenuB24[] = [
		GetMessage('TOP_MENU_PERFORMAN_MANAGEMENT'),
		'/performan/',
		[
			'/performan/',
		],
		[
			'menu_item_id' => 'menu_performan_management',
			'class' => 'menu-performan-library',
		],
		'',
	];

	$arMenuB24[] = [
		GetMessage('TOP_MENU_PERFORMAN_LIBRARY'),
		'/performan/library/',
		[
			'/performan/library/',
		],
		[
			'menu_item_id' => 'menu_performan_library',
		],
		'',
	];
}
